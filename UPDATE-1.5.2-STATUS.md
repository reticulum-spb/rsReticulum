...
## Финальная сверка: замена ожидающего announce по destination

Сверка Python `d825e393` / `40a862ac` выявила, что Rust безусловно добавлял
announce в per-interface queue, даже когда тот же destination уже ожидал
отправки. Теперь находится первая запись destination и обновляется на месте
только при более новом emission timestamp из random_hash: raw, hops и время
постановки заменяются вместе. Повторный/старый announce не продлевает ожидание
и не занимает дополнительное место. Другие destinations не изменяются.
Разбор учитывает размер Header1/Header2 и наличие ratchet; повторной проверки
подписи нет, так как это внутренняя очередь уже обработанных announces.
Неразбираемый вход не ставится в очередь. Политика переполнения для разных
destinations и интервал announce cap в этом блоке не изменены.

Существующий inline case проверяет повтор, замену более новым announce,
отказ от возврата к старому, сохранение соседнего destination и последующий
drain с announce cap — 1 passed (0.00s).

Дополнительно сверено Python `1bad7f58`: local Link interface binding в Rust
проверяется до dedup и повторно при dispatch; для transit links запись hash
отложена до успешного claim. Отвергнутый пакет не записывается ни в текущее,
ни в предыдущее поколение, поэтому отдельное удаление hash из обоих поколений
для этого случая не требуется. Короткая существующая проверка выявила старые
...
## Финальная сверка: частота исходящих announces после TX admission

Выпуск из per-interface announce queue обновлял byte/packet counters, но
не добавлял отсчёт в `outgoing_announce_frequency`. Прямые и attached пути,
наоборот, делали такой вызов после попытки отправки независимо от её результата.
Теперь отсчёт записывается в общей успешной ветке `send_to_interface` рядом
с control traffic counters, а прежние внешние вызовы удалены. Это покрывает
также отложенный выпуск без двойного учёта прямых отправок.

Под успешной отправкой здесь понимается принятие в driver TX queue, не
подтверждённая доставка по физическому каналу. Full/closed queue и запрет
outbound не дают отсчёта частоты. Размеры counters по-прежнему считаются до
IFAC. Семантика PR frequency и удаления отложенного announce при отказе
драйверной очереди в этом блоке не изменена.

Расширены существующие inline checks: один принятый announce и последующие
отказы не создают частоту из двух отсчётов; вторая принятая отправка создаёт.
