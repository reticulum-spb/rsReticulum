...
| `announce_batch_delay_ms` | integer | `10` | Ordinary announce accumulation window in milliseconds (`1..=10000`). Starts with the first staged write and does not slide as more arrive. Barriers may flush earlier; commit and scheduling can add delay. Limits remain 32 mutations / 64 KiB, with the existing exception for a single large mutation. Restart required. |
